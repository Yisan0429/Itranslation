...
### 🚀 新增
- **译文问题族系统** — `src/defects.py`（知识） + `src/auditor.py`（引擎）
  - 10 个问题族：被字句/长定语/硬译句式/是...的滥用/名词化/代词不清/习语硬译/超长句/术语不一致/裸露源语词
  - 每个族含：症状、检测正则、风险、修复模式、示例
  - 低 token 审计：正则扫描命中项，按 P1/P2/P3 分级，支持采样去重后 LLM 深度审查
  - CLI 和 GUI 翻译完成后自动运行，零 API 开销
...
- **稳定性加固** — 单块 API 失败不再崩溃全流程
  ...
  - 错误汇总面板 + 重跑提示（自动跳过已翻译块）
- **CLI 进度可视化** — rich 进度条 + 预检估算
