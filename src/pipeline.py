...
def run_translation_pipeline(params:dict,log_fn=None,progress_fn=None,cancel_fn=None)->dict:
 ...
 progress(.02,'Phase 0: Extract + Pre-read'); text=extract_book(str(book),use_vision=not params.get('no_vision',False)); log(f'Extracted: {len(text)} chars'); text,ph=protect(text); chapters=parse_structure(text); log(f'Chapters: {len(chapters)}'); kg={}; glossary={}
