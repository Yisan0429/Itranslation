...
from vector_store import TranslationVectorStore, SafeVectorStore
...
def run_translation_pipeline(params:dict,log_fn=None,progress_fn=None,cancel_fn=None)->dict:
 log=log_fn or print; progress=progress_fn or (lambda f,m:log(m)); cancel=cancel_fn or (lambda:False); started=time.time(); cfg=params['config']; book=Path(params['book']); errors=[]; cfg.setdefault('_cost',{}); cfg['_cost'].setdefault('prompt_tokens',0); cfg['_cost'].setdefault('completion_tokens',0); cfg.setdefault('_timing',{'api_calls':0,'api_sec':0.0,'api_retries':0,'cache_hit_tokens':0,'count_retries':0}); cfg['_timing'].setdefault('phases',{})
 def mark_phase(name,t0):
  cfg['_timing']['phases'][name]=round(time.time()-t0,2); return time.time()
 ...
 t0=started
 progress(.03,'Extracting text...'); text=extract_book(str(book),use_vision=not params.get('no_vision',False),max_mb=cfg.get('max_input_file_mb',100),max_mb_abort=cfg.get('max_input_file_mb_abort',500)); text,ph=protect(text); chapters=parse_structure(text); log(f'Extracted: {len(text)} chars, {len(chapters)} chapters'); kg={}; glossary={}; t0=mark_phase('extract',t0)
 ...
 total=sum(len(x) for _,x in groups); chars=sum(len(c.text) for _,xs in groups for c in xs); progress(.07,f'Translating ({total} chunks)'); shared=ConsistencyModel(threshold=cfg.get('consistency_alert_threshold',0.8)); results=[]; vs=None; t0=mark_phase('chunk',t0)
 if not params.get('no_rat',False):
  ...
  if not vs.ready:
   ...
   vs=SafeVectorStore(vs)  # 多章并行时单写队列落库，检索只读
 ...
 if workers==0 and len(groups)>1: workers=min(len(groups),cfg.get('max_parallel_workers',8))
 try:
  ...
  if vs is not None: vs.close()  # 等待单写队列排空
 t0=mark_phase('translate',t0)
 if errors:
 ...
 progress(.96,'Quality audit...'); issues=shared.audit_all(min_occurrences=3); rd=Path(cfg['reports_dir'])/'consistency'; rd.mkdir(parents=True,exist_ok=True); log(generate_consistency_report(issues,shared.get_glossary_snapshot(),output_path=str(rd/'consistency_report.txt'),threshold=cfg.get('consistency_alert_threshold',0.8))); shared.save(str(rd/'consistency_model.json')); json.dump(shared.get_glossary_snapshot(),open(rd/'glossary_final.json','w',encoding='utf8'),ensure_ascii=False,indent=2); t0=mark_phase('audit',t0)
 progress(.99,'Assembling...'); name=book.stem; out=params.get('output') or str(PROJECT_ROOT/'output'/name/f"{name}.{params.get('format','txt')}"); Path(out).parent.mkdir(parents=True,exist_ok=True); strict=cfg.get('strict_assembly',False); mm=sum(sentence_mismatch_count(c,tr) for t,c,tr in results); assemble_book([(t,restore(assemble_translations(c,tr,strategy=cfg.get('assembly_strategy','first_lock'),strict=strict),ph,verbose=False)) for t,c,tr in results],out,fmt=params.get('format','txt')); progress(1,'Translation complete'); _ps['display']=1.0; mark_phase('assemble',t0); c=cfg.get('_cost',{}); r=_result(out,chapters,groups,issues,errors,started,cfg,mm); log(_speed_line(r,chars)); return r
def _result(out,chapters,groups,issues,errors,started,cfg,mismatches=0):
 c=cfg.get('_cost',{}); t=cfg.get('_timing',{}); return {'output_path':out,'num_chapters':len(chapters),'num_chunks':sum(len(x) for _,x in groups),'num_issues':len(issues),'num_errors':len(errors),'elapsed_sec':time.time()-started,'prompt_tokens':c.get('prompt_tokens',0),'completion_tokens':c.get('completion_tokens',0),'errors':errors,'sentence_count_mismatches':mismatches,'api_calls':t.get('api_calls',0),'api_sec':t.get('api_sec',0),'api_retries':t.get('api_retries',0),'count_retries':t.get('count_retries',0),'cache_hit_tokens':t.get('cache_hit_tokens',0),'phases':dict(t.get('phases',{}))}
def _speed_line(r,chars):
 t=r.get('phases',{}); sec=t.get('translate',r['elapsed_sec']) or 1
 cpm=chars/max(sec,1)*60; ckpm=r['num_chunks']/max(sec,1)*60; tpm=(r['prompt_tokens']+r['completion_tokens'])/max(sec,1)*60
 return f"Speed: {int(cpm):,} chars/min | {ckpm:.1f} chunks/min | {int(tpm):,} tok/min | API {r['api_calls']} calls, {r['api_sec']:.0f}s busy, {r['api_retries']} transport retries, {r['count_retries']} count-retries, cache-hit {r['cache_hit_tokens']:,} tok | phases {t}"
